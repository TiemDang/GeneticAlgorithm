...
class population:
    ...
    def selection(self, fitness_list, population):
        ...
        rest_of_newpop = np.random.randint(0,10,size=(self.population_size - len(new_population), self.num_chromo, self.num_gene))
...
"""
# Run 
method = initialize([-1, 1],[-1, 1],4,2,6)
population = method.create_population()
decode_population = method.decode_gen(population)
fitness_list = method.fitness(decode_population)
selection = method.selection(fitness_list, population)
cross_over = method.crossover(selection)
print(cross_over)
mutation = method.mutation(cross_over, 1)
#crossover = method.crossover(population)
print("----------"*3)
print(decode_population, len(decode_population))
"""
